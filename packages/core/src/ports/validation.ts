...
// Cross-distro validation rules (FR-1.3, FR-1.4): accepted by the config
// schema, rejected at runtime because the capability doesn't exist under
// the chosen distro.

// kumulo: k3s has no autoscaler yet (design §9) — schema accepts the block
...
export const validateAutoscaling = (config: ClusterConfigShape): Option.Option<PlanRejected> =>
  config.distro === "k3s" && config.workerPools.some((pool) => pool.autoscaling?.enabled === true)
