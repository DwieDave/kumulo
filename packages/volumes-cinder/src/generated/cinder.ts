...
export const VolumesCreateRequest = Schema.Struct({ "volume": Schema.Struct({ "name": Schema.optionalKey(Schema.String), "size": Schema.Number.check(Schema.isInt()), "volume_type": Schema.optionalKey(Schema.String), "metadata": Schema.optionalKey(Schema.Record(Schema.String, Schema.String)) }) })
export type VolumeRecord = { readonly "id": string, readonly "name"?: string, readonly "metadata"?: { readonly "kumulo_cluster"?: string } }
export const VolumeRecord = Schema.Struct({ "id": Schema.String.check(Schema.isMinLength(1)), "name": Schema.optionalKey(Schema.String), "metadata": Schema.optionalKey(Schema.Struct({ "kumulo_cluster": Schema.optionalKey(Schema.String) })) })
...
export type VolumesDetailResponse = { readonly "volumes": ReadonlyArray<VolumeRecord>, readonly "volumes_links"?: ReadonlyArray<{ readonly "rel": string, readonly "href": string }> }
export const VolumesDetailResponse = Schema.Struct({ "volumes": Schema.Array(VolumeRecord), "volumes_links": Schema.optionalKey(Schema.Array(Schema.Struct({ "rel": Schema.String, "href": Schema.String }))) })
export type VolumesCreateResponse = { readonly "volume": VolumeRecord }
export const VolumesCreateResponse = Schema.Struct({ "volume": VolumeRecord })
// schemas
export type VolumesDetailGetParams = { readonly "limit"?: number, readonly "marker"?: string }
export const VolumesDetailGetParams = Schema.Struct({ "limit": Schema.optionalKey(Schema.Number.check(Schema.isInt())), "marker": Schema.optionalKey(Schema.String.annotate({ "format": "uuid" })) })
export type VolumesDetailGetQuery = { readonly "limit"?: number, readonly "marker"?: string }
export const VolumesDetailGetQuery = Schema.Struct({ "limit": Schema.optionalKey(Schema.Number.check(Schema.isInt())), "marker": Schema.optionalKey(Schema.String.annotate({ "format": "uuid" })) })
