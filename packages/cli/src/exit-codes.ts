...
/**
 * FR-10.3 — one distinct exit code per error family, so scripts/CI can
 * branch on failure kind without parsing the message. `0` is reserved for
 * success and never appears here; unlisted tags (defects escaping the
 * boundary) fall back to `1` in `exitCodeFor`.
 */
