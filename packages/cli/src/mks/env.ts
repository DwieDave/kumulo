...
const _required = (name: string): Effect.Effect<string, AuthenticationFailed> => {
...
export const MksEnvLive: Layer.Layer<MksEnv, AuthenticationFailed, HttpClient.HttpClient> = Layer.effect(
  MksEnv,
  Effect.gen(function*() {
    const clientId = yield* _required("OVH_CLIENT_ID")
    const clientSecret = yield* _required("OVH_CLIENT_SECRET")
    const serviceName = yield* _required("OVH_SERVICE_NAME")
