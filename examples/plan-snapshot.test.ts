...
import type { DesiredResource, Inventory } from "@kumulo/core"
...
const _config = Effect.runSync(parseConfigYaml(readFileSync(join(import.meta.dirname, "ovh-mks.yaml"), "utf8")))

const _desired: ReadonlyArray<DesiredResource> = _config.worker_pools.flatMap((pool) =>
  Array.from({ length: pool.count }, (_, index) => ({
    cluster: _config.name,
...
describe("dry-run plan against fake inventory", () => {
